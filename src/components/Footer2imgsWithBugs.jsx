...
const Footer = () => {
	...
	const getRandomImages = () => {
		...
		while (selectedImages.length < 2 && availableProjects.length > 0) {
			const projectIndex = Math.floor(Math.random() * availableProjects.length);
			const project = availableProjects.splice(projectIndex, 1)[0];

			if (project?.images && project.images.length > 0) {
				// Фільтрація зображень, щоб вибирати лише широкоформатні
				const wideImages = project.images.filter((image) => image.width >= image.height);

				if (wideImages.length > 0) {
					const randomImageIndex = Math.floor(Math.random() * wideImages.length);
					selectedImages.push({
						url: wideImages[randomImageIndex].url,
